 ...
 */
CREATE TABLE IF NOT EXISTS staging.enrollment_event (
    row_id INTEGER REFERENCES staging.event(row_id) PRIMARY KEY,
 ...
 */
CREATE TABLE IF NOT EXISTS staging.pace_event(
	row_id INTEGER REFERENCES staging.event(row_id) PRIMARY KEY,
	data_provider_id VARCHAR(50) NOT NULL,
    data_provider_patient_id VARCHAR(255) NOT NULL,
 ...
 */
CREATE TABLE IF NOT EXISTS staging.consent_event(
	row_id INTEGER REFERENCES staging.event(row_id) PRIMARY KEY,
	data_provider_id VARCHAR(50) NOT NULL,
    data_provider_patient_id VARCHAR(255) NOT NULL,
    patient_id CHAR(14) NOT NULL,
 ...
 * 1:1 with event
 */
CREATE TABLE IF NOT EXISTS staging.patient_address (
    row_id INTEGER REFERENCES staging.event(row_id) PRIMARY KEY,
 ...
 * 1:many with event
 */
CREATE TABLE IF NOT EXISTS staging.patient_communication (
    row_id INTEGER REFERENCES staging.event(row_id),
 ...
 * 1:1 with event 
 */
CREATE TABLE IF NOT EXISTS staging.patient_alternate_contact (
    row_id INTEGER REFERENCES staging.event(row_id) PRIMARY KEY,
 ...
 * 1:1 with event
 */
CREATE TABLE IF NOT EXISTS staging.legal_caregiver_or_guardian (
    row_id INTEGER REFERENCES staging.event(row_id) PRIMARY KEY,
 ...
 */
CREATE TABLE IF NOT EXISTS staging.legal_caregiver_or_guardian_address (
    row_id INTEGER REFERENCES staging.legal_caregiver_or_guardian(row_id) PRIMARY KEY,
 ...
 */
CREATE TABLE IF NOT EXISTS staging.legal_caregiver_or_guardian_communication (
    row_id INTEGER REFERENCES staging.legal_caregiver_or_guardian(row_id),
    value VARCHAR(100) NOT NULL,
    type VARCHAR(50) NOT NULL,
    is_primary BOOLEAN,
    status VARCHAR(50),
...
CREATE TABLE IF NOT EXISTS staging.consent (
	row_id SERIAL PRIMARY KEY,
    patient_row_id INTEGER REFERENCES staging.event(row_id),
 ...
 * 1:many with consent
 */
CREATE TABLE IF NOT EXISTS staging.consent_preference (
	row_id SERIAL PRIMARY KEY,
    consent_row_id INTEGER REFERENCES staging.consent(row_id),
    name VARCHAR(100) NOT NULL,
    created_at TIMESTAMP WITHOUT TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
 ...
 */
CREATE TABLE IF NOT EXISTS staging.consent_preference_option (
	row_id SERIAL PRIMARY KEY,
	consent_preference_row_id INTEGER REFERENCES staging.consent_preference(row_id),
 ...
 * 1:1 with event
 */
CREATE TABLE IF NOT EXISTS staging.clinical_detail (
    row_id INTEGER REFERENCES staging.event(row_id) PRIMARY KEY,
    icd_code VARCHAR(20),
 ...
 */
CREATE TABLE IF NOT EXISTS staging.prior_treatment (
	row_id SERIAL PRIMARY KEY,
	clinical_detail_row_id INTEGER REFERENCES staging.clinical_detail(row_id),
 ...
 * 1:many with clinical detail
 */
CREATE TABLE IF NOT EXISTS staging.drug_allergy (
	row_id SERIAL PRIMARY KEY,
	clinical_detail_row_id INTEGER REFERENCES staging.clinical_detail(row_id),
	allergy VARCHAR(50),
	created_at TIMESTAMP WITHOUT TIME ZONE DEFAULT CURRENT_TIMESTAMP
 ...
 */
CREATE TABLE IF NOT EXISTS staging.prescription (
    row_id INTEGER REFERENCES staging.clinical_detail(row_id) PRIMARY KEY,
 ...
 * 1:many with event
 */
CREATE TABLE IF NOT EXISTS staging.insurance_plan (
	row_id SERIAL PRIMARY KEY,
    patient_row_id INTEGER REFERENCES staging.event(row_id),
 ...
 * 1:many with event
 */
CREATE TABLE IF NOT EXISTS staging.insurance_type (
	row_id SERIAL PRIMARY KEY,
	patient_row_id INTEGER REFERENCES staging.event(row_id),
	type VARCHAR(255),
	created_at TIMESTAMP WITHOUT TIME ZONE DEFAULT CURRENT_TIMESTAMP
 ...
 * 1:1 with event
 */
CREATE TABLE IF NOT EXISTS staging.prescribing_hcp (
    row_id INTEGER REFERENCES staging.event(row_id) PRIMARY KEY,
 ...
 */
CREATE TABLE IF NOT EXISTS staging.prescribing_hcp_phone (
    row_id SERIAL PRIMARY KEY,
	prescribing_hcp_row_id INTEGER REFERENCES staging.prescribing_hcp(row_id),
 ...
 */
CREATE TABLE IF NOT EXISTS staging.prescribing_hcp_office_contact (
    row_id INTEGER REFERENCES staging.prescribing_hcp(row_id) PRIMARY KEY,
 ...
 */
CREATE TABLE IF NOT EXISTS staging.prescribing_hcp_office_address (
    row_id INTEGER REFERENCES staging.prescribing_hcp(row_id) PRIMARY KEY,
    address_line1 VARCHAR(50) NOT NULL,
    address_line2 VARCHAR(50),
    city VARCHAR(50) NOT NULL,
 ...
 * 1:1 with patient
 */
CREATE TABLE IF NOT EXISTS staging.primary_hcp (
    row_id INTEGER REFERENCES staging.event(row_id) PRIMARY KEY,
 ...
 */
CREATE TABLE IF NOT EXISTS staging.primary_hcp_phone (
	row_id SERIAL PRIMARY KEY,    
	primary_hcp_row_id INTEGER REFERENCES staging.primary_hcp(row_id),
    type VARCHAR(50) NOT NULL,
    is_primary BOOLEAN,
    status VARCHAR(50),
    number VARCHAR(15),
 ...
 */
CREATE TABLE IF NOT EXISTS staging.primary_hcp_office_address (
    row_id INTEGER REFERENCES staging.primary_hcp(row_id) PRIMARY KEY,
    address_line1 VARCHAR(50) NOT NULL,
    address_line2 VARCHAR(50),
    city VARCHAR(50) NOT NULL,
    state_or_province_code VARCHAR(3) NOT NULL,
 ...
 * 1:1 with event
 */
CREATE TABLE IF NOT EXISTS staging.generic_subscription_basic (
    row_id INTEGER REFERENCES staging.event(row_id) PRIMARY KEY,
 ...
 * 1:1 with event
 */
CREATE TABLE IF NOT EXISTS staging.case_management_service (
	row_id INTEGER REFERENCES staging.event(row_id) PRIMARY KEY,
    service_type VARCHAR(255) NOT NULL,
 ...
 * 1:1 with event
 */
CREATE TABLE IF NOT EXISTS staging.supply_service (
	row_id INTEGER REFERENCES staging.event(row_id) PRIMARY KEY,
    service_type VARCHAR(255) NOT NULL,
    subscription_status VARCHAR(255),
    subscription_date DATE,
 ...
 * 1:1 with event
 */
CREATE TABLE IF NOT EXISTS staging.copay_service (
	row_id INTEGER REFERENCES staging.event(row_id) PRIMARY KEY,
    service_type VARCHAR(255) NOT NULL,
    subscription_status VARCHAR(255) NOT NULL,
    subscription_date DATE,
 ...
 * 1:1 with event
 */
CREATE TABLE IF NOT EXISTS staging.patient_education_service (
	row_id INTEGER REFERENCES staging.event(row_id) PRIMARY KEY,
	service_type VARCHAR(255) NOT NULL,
    subscription_status VARCHAR(255),
    subscription_date DATE,
 ...
 */
CREATE TABLE IF NOT EXISTS staging.patient_education_service_education_type (
	row_id SERIAL PRIMARY KEY,
	patient_education_service_row_id INTEGER REFERENCES staging.patient_education_service(row_id),
	TYPE VARCHAR(255),
	created_at TIMESTAMP WITHOUT TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
 ...
 * 1:1 with event
 */
CREATE TABLE IF NOT EXISTS staging.rx_bridge_service (
    row_id INTEGER REFERENCES staging.event(row_id) PRIMARY KEY,
    service_type VARCHAR(255) NOT NULL,
    subscription_status VARCHAR(255),
    subscription_date DATE,
 ...
 */
CREATE TABLE IF NOT EXISTS staging.rx_bridge_service_prescription (
    row_id INTEGER REFERENCES staging.rx_bridge_service(row_id) PRIMARY KEY,
    drug_name VARCHAR(100) NOT NULL,
    drug_group VARCHAR(100),
    drug_form VARCHAR(255),
    drug_strength VARCHAR(255),
